...
qboolean SnapNormal( vec3_t normal ){
	...
	for ( i = 0; i < 3; i++ )
	{
		if ( normal[i] != 0.0 && -0.00005 < normal[i] && normal[i] < 0.00005 ) {
			normal[i] = 0.0;
			adjusted = true;
		}
	}
...
void SnapPlaneImproved( vec3_t normal, vec_t *dist, int numPoints, const vec3_t *points ){
	...
	if ( VectorIsOnAxis( normal ) ) {
		...
		if ( -0.005 < *dist - distNearestInt && *dist - distNearestInt < 0.005 ) {
			*dist = distNearestInt;
		}
	}
}
...

{
	...
	for ( i = -1; i <= 1; i++ )
	{
		...
		for (y = 0, p = mapplanes; y < nummapplanes; y++, p++)
		{
			...
			for ( j = 0; j < numPoints; j++ )
			{
				...
				// point number is greatly decreased.  The 0.005 cannot be
...
//                                      || we == WE_NONCONVEX
      ...
      {
        Log_Write("MarkBrushBevels: brush %d %s", brush->brushnum,
                  WindingErrorString());
        s->flags |= SFL_BEVEL;
      }                         //end else if
    }                           //end else
...
qboolean WriteMapBrush(FILE * fp, mapbrush_t * brush, vec3_t origin)
{
	...
	for (sn = 0; sn < brush->numsides; sn++)
	{
		...
		s->winding = (w ? CopyWindingAccuToRegular(w) : NULL);
	...
	if (fprintf
	(fp, "( %.3f %.3f %.3f ) ", w->p[2][0], w->p[2][1],
		w->p[2][2]) < 0)
		return false;
/*
    if (fprintf
        (fp, "  ( %5i %5i %5i ) ", (int) w->p[p1][0], (int) w->p[p1][1],
         (int) w->p[p1][2]) < 0)
      return false;
    ...
    if (fprintf
        (fp, "( %5i %5i %5i ) ", (int) w->p[2][0], (int) w->p[2][1],
         (int) w->p[2][2]) < 0)
      return false;
*/
    //free the winding
    ...
    if (s->texinfo == TEXINFO_NODE)
    {
      if (brush->contents & CONTENTS_PLAYERCLIP)
      {
        ...
        {
          if (fprintf(fp, "common/clip 0 0 0 1 1 1 0 0") < 0)
            return false;
        }
        else
        {
    ...
    // ML081030 modified to write correct texture name
    else if (loadedmaptype == MAPTYPE_QUAKE3)
    {
      char texbuf[256], *cp;
	...
	else
	{
		...
		ComputeAxisBase(plane, axis[0], axis[1]);
	...
	//	 (fp, "%s %d %d %d", ti->texture, shift[0], shift[1], rotate) < 0)
	// [ 0 0.707107 -0.707107 64 ] [ 0 -0.707107 -0.707107 32 ] 45 1 1
...
qboolean WriteOriginBrush(FILE * fp, vec3_t origin)
{
  ...
  for (i = 0; i < 3; i++)
  {
    for (s = -1; s <= 1; s += 2)
    {
      ...
      dist = origin[i] * s + 16;
      //
      w = BaseWindingForPlaneAccu(normal, dist);
      //three non-colinear points to define the plane
      if (fprintf
          (fp, "  ( %5i %5i %5i ) ", (int) w->p[0][0], (int) w->p[0][1],
           (int) w->p[0][2]) < 0)
        return false;
      if (fprintf
          (fp, "( %5i %5i %5i ) ", (int) w->p[1][0], (int) w->p[1][1],
           (int) w->p[1][2]) < 0)
        return false;
      if (fprintf
          (fp, "( %5i %5i %5i ) ", (int) w->p[2][0], (int) w->p[2][1],
...
void ResetMapLoading(void)
{
  ...
  memset(side_brushtextures, 0,
         MAX_MAPFILE_BRUSHSIDES * sizeof(brush_texture_t));
  //
  nummapplanes = 0;
...
int LoadMapFromBSP(struct quakefile_s *qf)
{
  ...
  size = ReadQuakeFile(qf, &idheader, 0, sizeof(idheader_t));

  if (size != sizeof(idheader_t))
    return false;
  ...
  // Sin BSP file
    ...
    if (
         (
           idheader.ident == SIN_BSPHEADER &&
           idheader.version == SIN_BSPVERSION
         )
         ||
             // the dorks gave the same format another ident and verions
         (
           idheader.ident == SINGAME_BSPHEADER &&
           idheader.version == SINGAME_BSPVERSION
         )
       )
  {
    ResetMapLoading();
  ...
  // Quake1 bsp files don't have a ident only a version
