...
//===========================================================================
void
Q1_SplitBrush(bspbrush_t * brush, int planenum, int nodenum,
...
bspbrush_t *Q1_CreateBrushes_r(bspbrush_t * brush, int nodenum)
{
  ...
  if (nodenum < 0)
  {
    ...
    case Q1_CONTENTS_CURRENT_DOWN:
      {
        Error("Q1_CreateBrushes_r: found contents %d in Half-Life BSP",
              leaf->contents);
        return NULL;
      }                         //end case
#endif /* HLCONTENTS */
    default:
      {
        Error("Q1_CreateBrushes_r: unknown contents %d in Half-Life BSP",
              leaf->contents);
        return NULL;
      }                         //end default
    }                           //end switch
  ...
  planenum =
    FindFloatPlane(q1_dplanes[planenum].normal, q1_dplanes[planenum].dist, 0, NULL);
      ...
      if (bestfacenum >= 0)
      {
        ...
        for (n = 0; n < 4; n++)
        {
          map_texinfo[texinfonum].vecs[0][n] =
            q1_texinfo[texinfonum].vecs[0][n];
          map_texinfo[texinfonum].vecs[1][n] =
            q1_texinfo[texinfonum].vecs[1][n];
        }                       //end for
