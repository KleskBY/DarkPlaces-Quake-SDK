...
void AppendPathSeperator(char *path, int length)
{
  ...
  if (strlen(path) && length - pathlen > 1 && path[pathlen - 1] != '/'
      && path[pathlen - 1] != '\\')
  {
...
qboolean
FindQuakeFile2(char *basedir, char *gamedir, char *filename,
               foundfile_t * file)
{
  int dir, i;
...
#else //BOTLIB
qboolean
FindQuakeFile(char *basedir, char *gamedir, char *filename,
              foundfile_t * file)
{
