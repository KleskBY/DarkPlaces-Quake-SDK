...
//===========================================================================
void
AAS_SplitFace(tmp_face_t * face, vec3_t normal, float dist,
              tmp_face_t ** frontface, tmp_face_t ** backface)
{
  ...
  *frontface = *backface = NULL;
  ...
  ClipWindingEpsilon(face->winding, normal, dist, FACECLIP_EPSILON, &frontw,
                     &backw);
...
winding_t *AAS_SplitWinding(tmp_area_t * tmparea, int planenum)
{
  ...
  for (face = tmparea->tmpfaces; face && splitwinding;
       face = face->next[side])
  {
...
//===========================================================================
int
AAS_TestSplitPlane(tmp_area_t * tmparea, vec3_t normal, float dist,
...
//===========================================================================
void
AAS_SplitArea(tmp_area_t * tmparea, int planenum, tmp_area_t ** frontarea,
              tmp_area_t ** backarea)
{
  int side;
...
/*
  ...
  for (face = tmparea->tmpfaces; face; face = nextface)
  {
    ...
    //split the face
    AAS_SplitFace(face, splitplane->normal, splitplane->dist, &frontface,
                  &backface);
...
//===========================================================================
int
AAS_FindBestAreaSplitPlane(tmp_area_t * tmparea, vec3_t normal, float *dist)
{
  int side1, side2;
  ...
  for (face1 = tmparea->tmpfaces; face1; face1 = face1->next[side1])
  {
    ...
    for (face2 = face1->next[side1]; face2; face2 = face2->next[side2])
    {
      ...
      if (!
          (((face1->faceflags & FACE_GROUND)
            && AAS_GapFace(face2, side2))
           || ((face2->faceflags & FACE_GROUND)
               && AAS_GapFace(face1, side1))))
        continue;
      //find a plane seperating the windings of the faces
      if (!FindPlaneSeperatingWindings
          (face1->winding, face2->winding, invgravity, tmpnormal, &tmpdist))
        continue;
#ifdef AW_DEBUG
                                              ...
                                              int planenum)
{
  ...
  //do the children recursively
  tmpnode->children[0] =
    AAS_RefreshLadderSubdividedTree_r(tmpnode->children[0], tmparea,
                                      tmpnode1, tmpnode2, planenum);
  tmpnode->children[1] =
    AAS_RefreshLadderSubdividedTree_r(tmpnode->children[1], tmparea,
                                      tmpnode1, tmpnode2, planenum);
  return tmpnode;
...
tmp_node_t *AAS_LadderSubdivideArea_r(tmp_node_t * tmpnode)
{
  ...
  if (tmparea->contents & (AREACONTENTS_WATER
                           | AREACONTENTS_LAVA | AREACONTENTS_SLIME))
    return tmpnode;
  ...
  //refresh the tree
  AAS_RefreshLadderSubdividedTree_r(tmpaasworld.nodes, tmparea, tmpnode1,
                                    tmpnode2, planenum);
  //
  return tmpnode;
