...
//===========================================================================
vec_t
BoxOriginDistanceFromPlane(vec3_t normal, vec3_t mins, vec3_t maxs, int side)
{
  vec3_t v1, v2;
  int i;
...
int AAS_AlwaysTriggered_r(char *targetname)
{
  ...
  for (i = 0; i < num_entities; i++)
  {
    ...
    if (!strcmp(targetname, ValueForKey(&entities[i], "target")))
    {
      ...
      if (!
          (atoi(ValueForKey(&entities[i], "spawnflags")) &
...
int AAS_ValidEntity(entity_t * mapent)
{
  ...
  else if (!strcmp("trigger_multiple", ValueForKey(mapent, "classname")))
  {
    ...
    for (i = 0; i < num_entities; i++)
    {
      ...
      {
        if (!strcmp
            ("target_teleporter", ValueForKey(&entities[i], "classname")))
        {
          return true;
        }                       //end if
      }                         //end if
...
//===========================================================================
void
AAS_CreateMapBrushes(mapbrush_t * brush, entity_t * mapent, int addbevels)
{
  int i;
  //side_t *s;
  ...
  else if (brush->contents & (CONTENTS_SOLID | CONTENTS_LADDER))
  {
    ...
    {
      AAS_ExpandMapBrush(bboxbrushes[i], cfg.bboxes[i].mins,
                         cfg.bboxes[i].maxs);
