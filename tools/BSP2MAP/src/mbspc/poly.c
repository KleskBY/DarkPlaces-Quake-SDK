...
winding_accu_t *BaseWindingForPlaneAccu( vec3_t normal, vec_t dist ){
	...
	// We're relying on the fact that 65536 is a power of 2 to keep
	...
	VectorScaleAccu( vright, ( (vec_accu_t) 65536 ) * 4.0, vright );

	// At time time of this writing, 65536 was 65536 (2^16).  Therefore
...
void ChopWindingInPlaceAccu( winding_accu_t **inout, vec3_t normal, vec_t dist, vec_t crudeEpsilon ){
	...
	static const vec_accu_t smallestEpsilonAllowed = ( (vec_accu_t) FLT_EPSILON ) * 0.5;
	...
	for ( i = 0; i < in->numpoints; i++ )
	{
		...
		if ( sides[i] == SIDE_ON || sides[i] == SIDE_FRONT ) {
			...
			if ( f->numpoints >= maxpts ) { // This will probably never happen.
				Log_Print("WARNING: estimate on chopped winding size incorrect (no problem)\n" );
...
*/
void
ChopWindingInPlace(winding_t ** inout, vec3_t normal, vec_t dist,
                   vec_t epsilon)
{
  winding_t *in;
  vec_t dists[MAX_POINTS_ON_WINDING + 4];
...
int WindingError(winding_t * w)
{
  ...
  for (i = 0; i < w->numpoints; i++)
  {
    ...
    for (j = 0; j < 3; j++)
    {
      if (p1[j] > BOGUS_RANGE || p1[j] < -BOGUS_RANGE)
      {
        sprintf(windingerror,
                "winding point %d BUGUS_RANGE \'%f %f %f\'", j, p1[0],
                p1[1], p1[2]);
...
int
PointOnWinding(winding_t * w, vec3_t normal, float dist, vec3_t point,
               int *spot)
{
  int i, j;
  vec3_t v1, v2;
...
//===========================================================================
int
FindPlaneSeperatingWindings(winding_t * w1, winding_t * w2, vec3_t dir,
...
int
WindingsNonConvex(winding_t * w1, winding_t * w2,
                  vec3_t normal1, vec3_t normal2, float dist1, float dist2)
{
  int i;
