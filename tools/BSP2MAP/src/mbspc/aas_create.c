...
int AAS_GapFace(tmp_face_t * tmpface, int side)
{
  ...
  return (DotProduct(invgravity, mapplanes[tmpface->planenum ^ side].normal)
          > cfg.phys_maxsteepness);
...
int AAS_GroundFace(tmp_face_t * tmpface)
{
  ...
  return (DotProduct(invgravity, mapplanes[tmpface->planenum].normal) >
          cfg.phys_maxsteepness);
...
//===========================================================================
void
AAS_AddFaceSideToArea(tmp_face_t * tmpface, int side, tmp_area_t * tmparea)
{
  int tmpfaceside;
...
void AAS_CheckArea(tmp_area_t * tmparea)
{
  ...
  for (face = tmparea->tmpfaces; face; face = face->next[side])
  {
    ...
    {
      Log_Write
        ("AAS_CheckArea: area %d face %d winding plane unequal to face plane\r\n",
         tmparea->areanum, face->num);
    }                           //end if
#endif /* L_DEBUG */
  }                             //end for
...
void AAS_FlipAreaFaces(tmp_area_t * tmparea)
{
  ...
  for (face = tmparea->tmpfaces; face; face = face->next[side])
  {
...
#ifdef L_DEBUG
    {
      ...
      {
        Log_Write
          ("area %d face %d winding plane unequal to face plane\r\n",
           tmparea->areanum, face->num);
      }                         //end if
    }
#endif
...
void AAS_CreateAreaSettings(void)
{
  ...
  for (tmparea = tmpaasworld.areas; tmparea; tmparea = tmparea->l_next)
  {
    ...
    tmparea->settings =
      (tmp_areasettings_t *) GetClearedMemory(sizeof(tmp_areasettings_t));
...
void AAS_CheckAreaSharedFaces(tmp_area_t * tmparea1, tmp_area_t * tmparea2)
{
  ...
  if (numsharedfaces > 1)
  {
    ...
    for (face1 = tmparea1->tmpfaces; face1; face1 = face1->next[side])
    {
      ...
      {
        Log_Write
          ("face %d, planenum = %d, face->frontarea = %d face->backarea = %d\r\n",
           face1->num, face1->planenum, face1->frontarea->areanum,
           face1->backarea->areanum);
      }                         //end if
    }                           //end if
  }                             //end if
