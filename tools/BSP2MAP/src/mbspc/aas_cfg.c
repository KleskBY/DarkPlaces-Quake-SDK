...
fielddef_t cfg_fields[] = {
  {"phys_gravitydirection", CFG_OFS(phys_gravitydirection),
   FT_FLOAT | FT_ARRAY, 3}
  ,
...
int LoadCfgFile(char *filename)
{
  ...
  while (PC_ReadToken(source, &token))
  {
    if (!stricmp(token.string, "bbox"))
    {
      ...
      if (!ReadStructure
          (source, &bbox_struct, (char *) &cfg.bboxes[cfg.numbboxes]))
      {
        FreeSource(source);
        return false;
      }                         //end if
  ...
  if (VectorLength(cfg.phys_gravitydirection) < 0.9
      || VectorLength(cfg.phys_gravitydirection) > 1.1)
  {
