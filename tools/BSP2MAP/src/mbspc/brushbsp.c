...
//===========================================================================
int
BrushOutOfBounds(bspbrush_t * brush, vec3_t mins, vec3_t maxs, float epsilon)
{
  int i, j, n;
  winding_t *w;
  side_t *side;
  ...
  for (i = 0; i < brush->numsides; i++)
  {
    ...
    for (j = 0; j < w->numpoints; j++)
    {
      for (n = 0; n < 3; n++)
      {
        if (w->p[j][n] < (mins[n] + epsilon)
            || w->p[j][n] > (maxs[n] - epsilon))
          return true;
      }                         //end for
    }                           //end for
  }                             //end for
...
//===========================================================================
int
TestBrushToPlanenum(bspbrush_t * brush, int planenum,
...
//                                      continue;       // only check visible faces on first pass
        ...
        {
          s =
            TestBrushToPlanenum(test, pnum, &bsplits, &hintsplit,
                                &epsilonbrush);
...
void
SplitBrush(bspbrush_t * brush, int planenum,
           bspbrush_t ** front, bspbrush_t ** back)
{
	bspbrush_t     *b[2];
	int i, j;
	winding_t   *w, *cw[2], *midwinding;
	winding_accu_t *w_accu;
...
//===========================================================================
void
SplitBrushList(bspbrush_t * brushes,
...
void BuildTreeThread(int threadid)
{
  ...
  for (node = NextNodeFromList(); node;)
  {
    ...
    //split the brush list in two for both children
    SplitBrushList(brushes, node, &node->children[0]->brushlist,
                   &node->children[1]->brushlist);

    CheckBrushLists(node->children[0]->brushlist,
                    node->children[1]->brushlist);
    //free the old brush list
    FreeBrushList(brushes);
    node->brushlist = NULL;
