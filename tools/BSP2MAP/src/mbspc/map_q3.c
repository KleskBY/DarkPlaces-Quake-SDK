...
int Q3_BrushContents(mapbrush_t * b)
{
  ...
  if (mixed)
  {
    Log_Write("Entity %i, Brush %i: mixed face contents ", b->entitynum,
              b->brushnum);
    PrintContents(contents);
    Log_Write("\r\n");
    //
...
void Q3_DPlanes2MapPlanes(void)
{
  ...
  {
    dplanes2mapplanes[i] =
      FindFloatPlane(q3_dplanes[i].normal, q3_dplanes[i].dist, 0, NULL);
  }                             //end for
...
void Q3_ParseBSPBrushes(entity_t * mapent)
{
  ...
  {
    Q3_BSPBrushToMapBrush(&q3_dbrushes
                          [q3_dmodels[mapent->modelnum].firstBrush + i],
                          mapent);
  }                             //end for
...
void AAS_CreateCurveBrushes(void)
{
  ...
  for (i = 0; i < q3_numDrawSurfaces; i++)
  {
    ...
    if (!
        (q3_dshaders[surface->shaderNum].
         contentFlags & (CONTENTS_SOLID | CONTENTS_PLAYERCLIP)))
    {
    ...
    for (j = 0; j < pc->numFacets; j++)
    {
      ...
      //
      planenum =
        FindFloatPlane(pc->planes[facet->surfacePlane].plane,
                       pc->planes[facet->surfacePlane].plane[3], 0, NULL);
      //
      ...
      {
        side->planenum =
          FindFloatPlane(mapplanes[planenum ^ 1].normal,
                         mapplanes[planenum ^ 1].dist + 1, 0, NULL);
        side->flags |= SFL_TEXTURED | SFL_VISIBLE;
      }                         //end else
      side->contents = CONTENTS_SOLID;
      ...
      //
      winding =
        BaseWindingForPlane(mapplanes[side->planenum].normal,
                            mapplanes[side->planenum].dist);
      for (n = 0; n < facet->numBorders; n++)
      {
        ...
        side->planenum =
          FindFloatPlane(pc->planes[facet->borderPlanes[n]].plane,
                         pc->planes[facet->borderPlanes[n]].plane[3], 0, NULL);
      ...
      for (n = 0; n < 3; n++)
      {
        ...
        if (brush->mins[n] < -MAX_MAP_BOUNDS
