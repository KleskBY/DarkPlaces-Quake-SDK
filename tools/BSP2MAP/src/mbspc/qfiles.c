...
quakefile_t *FindQuakeFilesInPak(char *pakfile, char *filter)
{
  ...
  if ((fread(&packheader, 1, sizeof(dpackheader_t), fp) !=
       sizeof(dpackheader_t)) || (packheader.ident != IDPAKHEADER
  ...
  if (packheader.ident == IDPAKHEADER)
  {
    ...
    if (!idpackfiles)
      Error("out of memory");
    //read the dir entry
    if (fread(idpackfiles, sizeof(dpackfile_t), numpackdirs, fp) !=
        numpackdirs)
    {
      fclose(fp);
      free(idpackfiles);
    ...
    packfiles =
      (dsinpackfile_t *) malloc(numpackdirs * sizeof(dsinpackfile_t));
    if (!packfiles)
      Error("out of memory");
  ...
  else                          //its a Sin pack file
  {
    ...
    if (!packfiles)
      Error("out of memory");
    //read the dir entry
    if (fread(packfiles, sizeof(dsinpackfile_t), numpackdirs, fp) !=
        numpackdirs)
    {
      fclose(fp);
      free(packfiles);
