...
AAS_FloodClusterAreas_r(int areanum, int clusternum)
{
  ...
  {
    if (!aasworld.
	reachability[aasworld.areasettings[areanum].firstreachablearea +
		     i].areanum)
    {
      continue;
    }				//end if
...
AAS_FloodClusterAreasUsingReachabilities(int clusternum)
{
  ...
  for (i = 1; i < aasworld.numareas; i++)
  {
    ...
    for (j = 0; j < aasworld.areasettings[i].numreachableareas; j++)
    {
      //the reachable area
      areanum =
	aasworld.reachability[aasworld.areasettings[i].
			      firstreachablearea + j].areanum;
      //if this area is a cluster portal
...
//===========================================================================
void
AAS_ConnectedAreas_r(int *areanums, int numareas, int *connectedareas,
		     int curarea)
{
...
//===========================================================================
int
AAS_GetAdjacentAreasWithLessPresenceTypes_r(int *areanums, int numareas,
					    int curareanum)
{
...
AAS_CheckAreaForPossiblePortals(int areanum)
{
  ...
  for (i = 0; i < numareas; i++)
  {
	...
	otherareanum = face->frontarea;
      ...
      if (aasworld.areasettings[otherareanum].
	  contents & AREACONTENTS_CLUSTERPORTAL)
	return 0;
...
AAS_FloodCluster_r(int areanum, int clusternum)
{
  ...
  {
    otherareanum =
      aasworld.reachability[aasworld.areasettings[areanum].
			    firstreachablearea + i].areanum;
    if (!otherareanum)
    {
      continue;
...
AAS_AddTeleporterPortals(void)
{
  ...
  for (ent = entities; ent; ent = ent->next)
  {
	...
	botimport.Print(PRT_ERROR,
			"teleporter without destination (%s)\n", target);
	continue;
      }				//end if
	...
	aasworld.areasettings[link->areanum].contents |=
	  AREACONTENTS_CLUSTERPORTAL | AREACONTENTS_TELEPORTAL;
      }				//end for
      //
      for (link = areas; link; link = link->next_area)
      {
