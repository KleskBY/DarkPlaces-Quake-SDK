...
AAS_WeaponJumpZVelocity(vec3_t origin, float radiusdamage)
{
  ...
  start[2] +=
    forward[2] * rocketoffset[0] + right[2] * rocketoffset[1] +
    rocketoffset[2];
...
//===========================================================================
void
AAS_Accelerate(vec3_t velocity, float frametime, vec3_t wishdir,
...
//===========================================================================
void
AAS_AirControl(vec3_t start, vec3_t end, vec3_t velocity, vec3_t cmdmove)
{
...
//===========================================================================
int
AAS_ClipToBBox(aas_trace_t * trace, vec3_t start, vec3_t end,
...
//#endif //AAS_MOVE_DEBUG
	...
	for (i = 0; i < numareas; i++)
	{
	  ...
	  {
	    if (aasworld.areasettings[areas[i]].
		contents & AREACONTENTS_TELEPORTER)
	    {
	      VectorCopy(points[i], move->endpos);
	      move->endarea = areas[i];
	      VectorScale(frame_test_vel, 1 / frametime, move->velocity);
      ...
      {
	if (AAS_ClipToBBox
	    (&trace, org, trace.endpos, presencetype, mins, maxs))
	{
	...
	if (plane->normal[2] == 0 && (jump_frame < 0 || n - jump_frame > 2))
	{
	  ...
	  steptrace =
	    AAS_TraceClientBBox(start, stepend, presencetype, entnum);
	  //
...
//#endif //AAS_MOVE_DEBUG
	...
	if (!step)
	{
	  ...
	  if (stopevent & SE_HITGROUNDDAMAGE)
	  {
		...
		frame_test_vel[2] > old_frame_test_vel[2] && !onground)
	    {
