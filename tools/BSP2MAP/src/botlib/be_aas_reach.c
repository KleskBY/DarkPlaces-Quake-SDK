...
//===========================================================================
int
AAS_GetJumpPadInfo(int ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs,
		   vec3_t velocity)
{
  int modelnum, ent2;
  ...
  for (ent2 = AAS_NextBSPEntity(0); ent2; ent2 = AAS_NextBSPEntity(ent2))
  {
	...
	(ent2, "targetname", targetname, MAX_EPAIRKEY))
      continue;
...
AAS_BestReachableFromJumpPadArea(vec3_t origin, vec3_t mins, vec3_t maxs)
{
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
  {
    ...
    AAS_ClientMovementHitBBox(&move, -1, areastart, PRESENCE_NORMAL,
			      qfalse, velocity, cmdmove, 0, 30, 0.1f,
...
//===========================================================================
int
AAS_BestReachableArea(vec3_t origin, vec3_t mins, vec3_t maxs,
		      vec3_t goalorigin)
{
  int areanum, i, j, k, l;
  aas_link_t *areas;
...
AAS_SetupReachabilityHeap(void)
{
  ...
  reachabilityheap =
    (aas_lreachability_t *) GetClearedMemory(AAS_MAX_REACHABILITYSIZE *
...
AAS_MaxJumpHeight(float phys_jumpvel)
{
  ...
  return 0.5 * phys_gravity * (phys_jumpvel / phys_gravity) * (phys_jumpvel /
							       phys_gravity);
...
{
  return (aasworld.areasettings[areanum].
	  contents & AREACONTENTS_CLUSTERPORTAL);
...
//===========================================================================
int
AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int area1num,
						     int area2num)
{
...
//                                              Log_Write("lines no overlap: from area %d to %d\r\n", area1num, area2num);
  ...
  if (water_foundreach)
  {
    ...
    if (aasworld.areasettings[AAS_PointAreaNum(testpoint)].
	areaflags & AREA_LIQUID)
    {
	...
	if ((aasworld.areasettings[area1num].
	     presencetype & PRESENCE_NORMAL)
	    && (aasworld.areasettings[area2num].
		presencetype & PRESENCE_NORMAL))
	{
  ...
  if (ground_foundreach)
  {
	...
	if (!trace.startsolid && trace.fraction >= 1.0)
	{
	  ...
	  if (AAS_PointAreaNum(trace.endpos) == area2num)
	  {
	    ...
	    numareas =
	      AAS_TraceAreas(start, end, areas, NULL,
...
AAS_Reachability_Jump(int area1num, int area2num)
{
  ...
  vec3_t teststart, testend, dir, velocity, cmdmove, up =
    { 0, 0, 1 }, sidewards;
  aas_area_t *area1, *area2;
  aas_face_t *face1, *face2;
  aas_edge_t *edge1, *edge2;
  ...
  for (i = 0; i < area1->numfaces; i++)
  {
	...
	for (l = 0; l < face2->numedges; l++)
	{
	  ...
	  //
	  bestdist =
	    AAS_ClosestEdgePoints(v1, v2, v3, v4, plane1, plane2,
				  beststart, bestend, beststart2,
				  bestend2, bestdist);
	}			//end for
      }				//end for
    }				//end for
...
//              Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, bestdist);
    ...
    else
    {
      ...
      if (!AAS_HorizontalVelocityForJump
	  (phys_jumpvel, beststart, bestend, &speed))
	return qfalse;
      speed *= 1.05f;
	...
	if (!
	    (AAS_PointContents(trace.endpos) &
	...
	return qfalse;
      ...
      teststart[2] += 1;
      numareas =
	AAS_TraceAreas(move.endpos, teststart, areas, NULL,
		       sizeof(areas) / sizeof(int));
    ...
    {
      lreach->traveltime =
	aassettings.rs_startjump + VectorDistance(bestend,
						  beststart) * 240 /
...
//#endif REACH_DEBUG
	...
	continue;
      ...
      {
	if (!AAS_ValueForBSPEpairKey
	    (dest, "classname", classname, MAX_EPAIRKEY))
	  continue;
	if (!strcmp(classname, "target_teleporter"))
	{
	  if (!AAS_ValueForBSPEpairKey
	      (dest, "targetname", targetname, MAX_EPAIRKEY))
	    continue;
	  if (!strcmp(targetname, target))
	  {
	    break;
	...
	for (n = 0; n < 3; n++)
	{
	  ...
	  for (j = 0; j < 8; j++)
	  {
		...
		if (AAS_AreaGrounded(area2num) || AAS_AreaSwim(area2num))
		{
		  ...
		  trace =
		    AAS_TraceClientBBox(start, end, PRESENCE_CROUCH, -1);
		  if (trace.fraction >= 1)
		    break;
		}		//end if
	      }			//end if
...
//===========================================================================
aas_lreachability_t *
AAS_FindFaceReachabilities(vec3_t * facepoints,
			   int numpoints,
			   aas_plane_t * plane, int towardsface)
{
  int i, j, k, l;
  ...
  for (i = 1; i < aasworld.numareas; i++)
  {
	...
	for (l = 0; l < numpoints; l++)
	{
	  ...
	  dist =
	    AAS_ClosestEdgePoints(v1, v2, v3, v4, faceplane, plane,
				  beststart, bestend, beststart2,
				  bestend2, bestdist);
	  if (dist < bestdist)
	  {
...
AAS_Reachability_FuncBobbing(void)
{
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
  {
    ...
    //
    Log_Write
      ("funcbob model %d, start = {%1.1f, %1.1f, %1.1f} end = {%1.1f, %1.1f, %1.1f}\n",
       modelnum, move_start[0], move_start[1], move_start[2], move_end[0],
       move_end[1], move_end[2]);
    //
#ifndef BSPC
    /*
    ...
    for (i = 0; i < 2; i++)
    {
      ...
      {
	firststartreach =
	  AAS_FindFaceReachabilities(start_edgeverts, 4, &start_plane, qtrue);
	firstendreach =
	  AAS_FindFaceReachabilities(end_edgeverts, 4, &end_plane, qfalse);
      }				//end if
      else
      {
	firststartreach =
	  AAS_FindFaceReachabilities(end_edgeverts, 4, &end_plane, qtrue);
	firstendreach =
	  AAS_FindFaceReachabilities(start_edgeverts, 4, &start_plane,
				     qfalse);
      }				//end else
      //
	...
	for (endreach = firstendreach; endreach; endreach = nextendreach)
	{
	  ...
	  if (i == 0)
	    lreach->edgenum =
	      ((int) move_start[axis] << 16) | ((int) move_end[axis] &
						0x0000ffff);
	  else
	    lreach->edgenum =
	      ((int) move_end[axis] << 16) | ((int) move_start[axis] &
					      0x0000ffff);
...
AAS_Reachability_JumpPad(void)
{
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
  {
    ...
    if (velocity[0] || velocity[1])
    {
      ...
      for (i = 0; i < 20; i++)
      {
	AAS_PredictClientMovement(&move, -1, areastart,
				  PRESENCE_NORMAL, qfalse, velocity,
				  cmdmove, 0, 30, 0.1f,
				  SE_HITGROUND | SE_ENTERWATER |
				  SE_ENTERSLIME | SE_ENTERLAVA |
				  SE_HITGROUNDDAMAGE | SE_TOUCHJUMPPAD
				  | SE_TOUCHTELEPORTER, 0,
	...
	//speed
	ret =
	  AAS_HorizontalVelocityForJump(zvel, areastart, facecenter, &speed);
	if (ret && speed < 150)
	{
	  ...
	  dir[2] = 0;
	  {
	    ...
	    //
	    AAS_PredictClientMovement(&move, -1, areastart,
				      PRESENCE_NORMAL, qfalse,
				      velocity, cmdmove, 30, 30,
				      0.1f,
				      ...
				      SE_TOUCHJUMPPAD |
				      SE_TOUCHTELEPORTER |
				      SE_HITGROUNDAREA, area2num, visualize);
	    //if prediction time wasn't enough to fully predict the movement
	    //don't enter slime or lava and don't fall from too high
	    if (move.frames < 30 &&
		!(move.
		  stopevent & (SE_ENTERSLIME | SE_ENTERLAVA |
			       SE_HITGROUNDDAMAGE))
		&& (move.
		    stopevent & (SE_HITGROUNDAREA | SE_TOUCHJUMPPAD |
				 SE_TOUCHTELEPORTER)))
	    {
		...
		for (link = areas; link; link = link->next_area)
		{
		  ...
		  //NOTE: the edgenum is the horizontal velocity
		  lreach->edgenum =
		    sqrt(cmdmove[0] * cmdmove[0] + cmdmove[1] * cmdmove[1]);
		  VectorCopy(areastart, lreach->start);
...
AAS_Reachability_Grapple(int area1num, int area2num)
{
  ...
  for (i = 0; i < area2->numfaces; i++)
  {
    ...
    if (aasworld.areasettings[areanum].
	contents & (AREACONTENTS_SLIME | AREACONTENTS_LAVA))
    {
      continue;
    }				//end if
    ...
    for (j = 0; j < numareas; j++)
    {
      if (aasworld.areasettings[areas[j]].
	  contents & AREACONTENTS_CLUSTERPORTAL)
	break;
    }				//end for
    if (j < numareas)
...
//      }
  ...
  for (i = 0; i < area2->numfaces; i++)
  {
	...
	dir[2] = 0;
	{
	  ...
	  //
	  AAS_PredictClientMovement(&move, -1, areastart,
				    PRESENCE_NORMAL, qtrue, velocity,
				    cmdmove, 30, 30, 0.1f,
				    SE_ENTERWATER | SE_ENTERSLIME |
				    SE_ENTERLAVA | SE_HITGROUNDDAMAGE |
				    SE_TOUCHJUMPPAD | SE_HITGROUND |
				    SE_HITGROUNDAREA, area2num, visualize);
	  //if prediction time wasn't enough to fully predict the movement
	  //don't enter slime or lava and don't fall from too high
	  if (move.frames < 30 &&
  ...
  for (i = 0; i < area->numfaces; i++)
  {
	...
	for (l = 0; l < face2->numedges; l++)
	{
	  ...
	  if (abs(edge1num) == abs(edge2num))
	  {
	    ...
	    //
	    if (aasworld.areasettings[reachareanum].
		contents & (AREACONTENTS_SLIME | AREACONTENTS_LAVA))
	    {
	    ...
	    //if not going through a cluster portal
	    numareas =
	      AAS_TraceAreas(mid, testend, areas, NULL,
			     sizeof(areas) / sizeof(int));
	    ...
	    lreach->traveltype = TRAVEL_WALKOFFLEDGE;
	    lreach->traveltime =
	      aassettings.rs_startwalkoffledge + fabs(mid[2] -
						      trace.
						      endpos[2]) *
...
AAS_StoreReachability(void)
{
  ...
  aasworld.reachability =
    (aas_reachability_t *) GetClearedMemory((numlreachabilities + 10) *
...
AAS_ContinueInitReachability(float time)
{
       ...
       i++)
  {
	...
	continue;
      ...
      if (aasworld.areasettings[i].
	  contents & (AREACONTENTS_TELEPORTER | AREACONTENTS_JUMPPAD))
      {
	if (!
	    (aasworld.areasettings[j].
	     contents & (AREACONTENTS_TELEPORTER | AREACONTENTS_JUMPPAD)))
	{
	  continue;
	}			//end if
...
AAS_InitReachability(void)
{
  ...
  areareachability =
    (aas_lreachability_t **) GetClearedMemory(aasworld.numareas *
					      sizeof(aas_lreachability_t *));
  //
